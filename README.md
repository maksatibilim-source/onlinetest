...
### 🧹 Дерекқорды бір рет толық тазалау (RESET_DB)

Кейде схема өзгерісі аддитивті емес (мыс. бар кестеге міндетті баған қосу).
Ондайда `db push` қатемен тоқтайды («cannot be executed»). Дерек әлі маңызды
болмаса, дерекқорды бір рет тазалаңыз:

1. App сервисі → Variables → `RESET_DB` = `true` қой.
2. Redeploy — старт кезінде дерекқор ТОЛЫҚ тазаланып, жаңа схемамен құрылады.
3. **Деплой сәтті өткен соң `RESET_DB` айнымалысын ӨШІРІҢІЗ** (әйтпесе әр
   деплойда дерек өшеді!).
